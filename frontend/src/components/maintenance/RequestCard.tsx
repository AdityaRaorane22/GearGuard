...
const RequestCard: React.FC<RequestCardProps> = ({ request, onClick, className }) => {
  ...
  const avatarColor = request.assignedTechnician
    ? `hsl(${request.assignedTechnician.id.charCodeAt(0) * 10 % 360}, 70%, 60%)`
    : "hsl(210, 15%, 35%)";
  ...
  return (
    <>
      <div
        onClick={handleClick}
        className={[
          "bg-slate-900 rounded-lg p-3 text-sm cursor-pointer transition-all",
          "hover:shadow-md hover:shadow-emerald-500/20 hover:-translate-y-0.5",
          request.isOverdue ? "border-2 border-rose-500" : "border border-slate-800",
          className || "",
        ].join(" ")}
      >
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="min-w-0 flex-1">
            <div className="font-medium truncate text-slate-100">{request.subject}</div>
            <div className="text-xs text-slate-400 truncate">
              {request.equipment?.equipmentName || "Unknown Equipment"}
            </div>
          </div>
          {request.isOverdue && <AlertCircle className="w-4 h-4 text-rose-400 flex-shrink-0 mt-0.5" />}
        </div>
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-800 text-xs whitespace-nowrap">
        ...
        {request.isOverdue && (
          <div className="mt-1 text-xs text-rose-300 font-medium">⚠️ Overdue</div>
        )}
      </div>
          ...
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="text-sm font-medium capitalize">{request.stage}</div>
              </div>
              ...
              {request.maintenanceTeam && (
                <div>
                  ...
                  <div className="text-sm font-medium">{request.maintenanceTeam.teamName}</div>
                </div>
              )}
              {request.scheduledDate && (
                <div>
                  <div className="text-xs text-slate-400 mb-1">Scheduled Date</div>
                  <div className="text-sm font-medium">
                    {new Date(request.scheduledDate).toLocaleDateString()}
                  </div>
                </div>
              ...
              {request.durationHours && (
                <div>
                  ...
                  <div className="text-sm font-medium">{request.durationHours} hours</div>
                </div>
              )}
              {request.createdAt && (
                <div>
            ...
            {request.isOverdue && (
              <div className="p-3 rounded-lg bg-rose-900/30 border border-rose-800">
